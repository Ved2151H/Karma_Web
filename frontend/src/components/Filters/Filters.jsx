...
function Filters({
  ...
  const currentCat = categories.find((cat) => cat.slug === category);
  const subcategoriesList = currentCat ? currentCat.subcategories || [] : [];
  ...
  const getSubsectionCount = (subVal, ssVal) => {
    return allProducts.filter(p => p.category === category && p.subcategory === subVal && p.subsection === ssVal).length;
  };

  return (
    <div className="flex flex-col gap-1 pr-1 font-sans select-none">
      
      {/* SECTION 1: Subcategory & Subsection */}
      {subcategoriesList.length > 0 && (
        <FilterSection title="Category" isExpanded={expanded.subcategory} onToggle={() => toggleAccordion('subcategory')}>
          <div className="flex flex-col gap-3.5 text-xs text-neutral-700 font-medium">
            {subcategoriesList.map((sub) => {
              const hasSubsections = sub.subsections && sub.subsections.length > 0;
              return (
                <div key={sub.id} className="flex flex-col gap-2">
                  <label className="flex items-center gap-2 cursor-pointer hover:text-brand-red">
                    ...
                    />
                    <span className="font-bold text-neutral-800">{sub.name} ({getSubcategoryCount(sub.slug)})</span>
                  </label>
                  
                  {hasSubsections && (
                    <div className="pl-5 flex flex-col gap-2 border-l border-neutral-200">
                      {sub.subsections.map((ss) => (
                        <label key={ss.id} className="flex items-center gap-2 cursor-pointer hover:text-brand-red text-[11px] text-neutral-500">
                          <input
                            type="checkbox"
                            checked={selections.subsections?.includes(ss.slug) || false}
                            onChange={() => toggleSubsection(ss.slug)}
                            className="rounded border-neutral-350 text-brand-red focus:ring-brand-red w-3 h-3 cursor-pointer"
                          />
                          <span>{ss.name} ({getSubsectionCount(sub.slug, ss.slug)})</span>
                        </label>
                      ))}
                    </div>
                  )}
